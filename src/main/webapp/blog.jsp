...
<head>
  ...
  <title>Blog | Celine Closet</title>
...
<body>
  ...
  <header class="main-header" id="top">
    ...
    <nav class="nav" aria-label="Điều hướng chính">
      ...
      <a class="active-link" href="<%= ctx %>/blog">Blog</a>
  ...
  <main class="content-page">
    <section class="listing-hero">
      <span class="tiny-title">Celine Journal</span>
      <h1>Blog</h1>
      <p>Các bài viết được chia trang để giao diện không bị kéo dài. Mỗi trang đang hiển thị tối đa <%= pageSize %> bài.</p>
    </section>

    <section class="listing-wrap">
      <div class="listing-info">
        <span>Trang <%= currentPage %>/<%= totalPages %></span>
        <span><%= totalItems %> bài viết</span>
      </div>

      <div class="post-grid">
        <% for (PostItem post : posts) { %>
          <article class="post-card">
            <a class="post-image" href="<%= h(post.getDetailUrl()) %>">
              <img src="<%= h(post.getImageUrl()) %>" alt="<%= h(post.getTitle()) %>" />
            </a>
            <div class="post-body">
              <span class="post-category"><%= h(post.getCategory()) %></span>
              <h2><a href="<%= h(post.getDetailUrl()) %>"><%= h(post.getTitle()) %></a></h2>
              <p><%= h(post.getSummary()) %></p>
              <a class="read-more" href="<%= h(post.getDetailUrl()) %>">Xem thêm</a>
            </div>
          </article>
        <% } %>
      </div>
      ...
      <% if (totalPages > 1) { %>
        <nav class="pagination" aria-label="Phân trang blog">
          <a class="page-arrow <%= currentPage == 1 ? "disabled" : "" %>" href="<%= currentPage == 1 ? "#" : ctx + "/blog?page=" + (currentPage - 1) %>">«</a>
          <% for (int i = 1; i <= totalPages; i++) { %>
            <a class="page-number <%= i == currentPage ? "active" : "" %>" href="<%= ctx %>/blog?page=<%= i %>"><%= i %></a>
          <% } %>
          <a class="page-arrow <%= currentPage == totalPages ? "disabled" : "" %>" href="<%= currentPage == totalPages ? "#" : ctx + "/blog?page=" + (currentPage + 1) %>">»</a>
        </nav>
      <% } %>
    </section>
  </main>
